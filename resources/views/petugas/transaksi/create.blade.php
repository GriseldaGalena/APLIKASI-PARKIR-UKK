<x-layouts.app :title="__('Kendaraan Masuk')">
    ...
    <div class="mt-6 bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 p-8">
        <form action="{{ route('petugas.transaksi.store') }}" method="POST" class="space-y-6">
            ...
            {{-- Pilih Kendaraan --}}
            <div>
                <label for="id_kendaraan" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                ...
                @enderror
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Pilih kendaraan yang akan masuk parkir.</p>
            </div>

            {{-- Pilih Tarif --}}
            <div>
                <label for="id_tarif" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                ...
                @error('id_tarif')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Pilih tarif sesuai jenis kendaraan.</p>
            </div>

            {{-- Pilih Area --}}
            <div>
                <label for="id_area" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                ...
                @error('id_area')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Pilih area parkir yang tersedia.</p>
            </div>
